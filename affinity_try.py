...
X_avg_flip=np.mean(X_flip, axis=0)
def zscore(X1):
    X_z= np.zeros(X1.shape)
    for i in range(0,len(X)):
            X_z[i]= (X1[i]-np.mean(X1[i]))/np.std(X1[i])
    return X_z
test=zscore(X_avg)
X_avg_z = stats.zscore(X_avg)
X_flip_z = stats.zscore(X_avg_flip)
# test_z = stats.zscore(test,axis=1) if test is in size n*10000
#%%