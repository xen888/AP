...
def get_cluster_exampler_member(crit,X): # distance_i= pcc(exampler_i, examplar_k) for ith cluster
    ...
    for i in range(0,cluster_count):
        ...
        for k in range(0,i): #for examplars other than i
            ...
            # distance_1= math.dist(Examplar_i, Examplar_k) #
        ...
        for k in range(i+1, cluster_count):
            Examplar_k=X[keysList[k]]
            # distance_2= math.dist(Examplar_i, Examplar_k) # #distance_2= np.corrcoef(Examplar_i, Examplar_k)
            distance_2 = (event.pcc(Examplar_i,Examplar_k)) + 1
...
def get_cluster_rank(X,crit,t,fname): # X=whole N spectra, ind=label from clustering forn N spectra
     #get examplar and cluster member
    ...
    examDict, clusDict, distDict =get_cluster_exampler_member(crit,X)
    for k in range(0,cluster_count):
        ...
        X_cluster=X_cluster_clean
        avg_X= np.mean(X, axis=0)
        avg_X_norm= event.normalize(avg_X)
        avg_X_moving= event.moving_averages(avg_X_norm, 8) 
        
        
        Exam_spec = event.Event(0, X_ex)
        theory_spec = event.Event(0, t[0,:])
        norm_theory = event.normalize(theory_spec.data)
        norm_theory_moving = event.moving_averages(norm_theory, 8)
        ...
        avg_spec_moving= event.moving_averages(norm_exp, 8)
        avg_spec_moving_flipped = avg_spec_moving[::-1]
        norm_exp_k[k]=norm_exp
        
        # pcc_result_exam=event.pcc(norm_theory,norm_exam)
        # pcc_result_exam=str(round(pcc_result_exam, 3))
        
        pcc_result_mov_X=event.pcc(norm_theory_moving,avg_X_moving)
        pcc_result_mov_X=str(round(pcc_result_mov_X, 3))
        
        pcc_result_avg_cluster=event.pcc(norm_theory_moving, avg_spec_moving)
        pcc_result_avg_cluster=str(round(pcc_result_avg_cluster, 3))
        
        pcc_result_avg_cluster_flipped=event.pcc(norm_theory_moving, avg_spec_moving_flipped)
        pcc_result_avg_cluster_flipped=str(round(pcc_result_avg_cluster_flipped, 3))
        ...
        # plt.plot(avg_X_moving, label="X_Average_moving")
        plt.legend(loc='upper left', frameon=False, bbox_to_anchor=(0.3,0.9), ncol=2)
        plt.xlabel("Time ["+ str(fname)+"]")
        ...
        plt.title("Final PCC_value from Cluster_moving_average =%s and Cluster_moving_average_flipped = %s" %( pcc_result_avg_cluster,pcc_result_avg_cluster_flipped))
        plt.grid(False)
        ...
        for i in range(0,len(X_cluster)):
            # r_score = math.dist(X_cluster[i], X_ex)    # instead X_examplar we can use X_cluster average
            r_score = event.pcc(X_cluster[i],X_ex)-event.pcc(X_cluster[i],X_ex[::-1])-2
            r.append(r_score)
        a = np.mean(np.absolute(r)); b = np.mean(X_distance);
        rank_score[k]= a/b
    ...
    for k in range(0,cluster_count):
        pcc_result_cluster[k]= np.absolute(event.pcc(norm_theory,norm_exp_k[k]))
...
def DB_rank(X,crit):
    ...
    for i in range(0,cluster_count):
        cluster_members=list(crit.get(Examplar[index[i]]))
        member_count=len(cluster_members)
        test_all=[]
        for j in range(0, member_count):
            # x=X[Examplar[i]]; x= x[::-1]
            # test = math.dist(X[Examplar[i]],X[cluster_members[j]])  
            test = event.pcc(X[Examplar[i]],X[cluster_members[j]])+1 #correlation
            test_all.append(test)
        test_all=np.array(test_all)
        S[i] = np.mean(test_all)
        dist_all=[]
        for j in range(0, cluster_count):
            # dist_i_j = math.dist(X[Examplar[i]],X[Examplar[j]]) # 
            dist_i_j = event.pcc(X[Examplar[i]],X[Examplar[j]])+1 #correlation
    ...
    return ranks+1
def cluster_rank(X,crit):
    Examplar = list(crit.keys())
    Examplar = np.array(Examplar)
    cluster_count=len(Examplar)
    S = np.zeros([cluster_count])
    test = np.zeros([cluster_count])
    N=X.shape[1]
    avg_X = np.mean(X,axis=0)
    for i in range(0,cluster_count):
        cluster_members=list(crit.get(Examplar[i]))
        member_count=len(cluster_members)
        X_cluster=np.zeros([member_count,N])
        X_examplar= X[Examplar[i]]
        for k in range(0,member_count):
            cluster_index=cluster_members[k]
            X_cluster[k,:]=X[cluster_index]
        avg_cl = np.mean(X_cluster, axis=0)
        S[i]= event.pcc(avg_X,avg_cl)
        test[i] = event.pcc(avg_X,X_examplar)
    temp = (S*test).argsort() # ascending rank, lowest score rank 0
    ranks = np.arange(len(S))[temp.argsort()]
    ranks = ranks[::-1]
    return ranks+1
def get_plot(X,crit,ind,t, fname):
    ...
    ranks_cl_avg = cluster_rank(X,crit)
    ranks_sil, sil_avg = sil_cluster(X,ind)
    rank_score, pcc_score, ranks_ours, ranks_pcc = get_cluster_rank(X, crit, t, fname)
    # fig = plt.figure()
    # fig.set_figwidth(10)
    # fig.set_figheight(10)
    for fig_number in range(0,cluster_count):
        ...
        plt.title("Count ="+str(len(label_f))+ " PCC_rank="+str(ranks_pcc[fig_number]) + " Proposed rank="+str(ranks_ours[fig_number])+" DB_rank="+str(ranks_db[fig_number])+ " Sil_rank="+str(ranks_sil[fig_number])+" Avg_rank= "+str(ranks_cl_avg[fig_number]))    
        plt.xlabel("PCC(S,T)")
